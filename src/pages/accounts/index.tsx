...
import { useRoqClient, useAccountFindManyWithCount } from 'lib/roq';
...
export function AccountListPage(props: AccountListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useAccountFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.account.delete({ where: { id } });
      await mutate();
    } catch (error) {
