...
import { useRoqPlatformUserProfiles, useSubscriptionCount, useCoinDetailCount, useServiceCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const coinDetailStat = useCoinDetailCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Coin Details',
      result: coinDetailStat?.data,
      isLoading: coinDetailStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Services',
